...
data_dir = '/media/cheer/UI/Project/DarkPattern/code/individual_modules/statusCheck/Checkbox_random/test'
# print(glob(os.path.join(data_dir, "train", "**/**/**")))
save_root = "model_status_random3"
if not os.path.exists(save_root):
    os.makedirs(save_root)
classname_json = os.path.join(save_root, "statusModel_labels_random3.json")
...
class StatusDataset(Dataset):
    def __init__(self, root_dir, phase, transforms=None):
        self.root_dir = root_dir
        self.phase = phase
        self.classes = ["checked", "unchecked", "other"]
        self.class2id = {label:idx for idx, label in enumerate(self.classes)}
        # print(glob(os.path.join(self.root_dir, self.phase, "**/**/**"))
        # print(glob(os.path.join(self.root_dir, self.phase, "**/**/**")))
        self.all_data = glob(os.path.join(self.root_dir, self.phase, "**/**jpg"), recursive = True)
        ...
        if self.phase == "test":
            self.all_data = [d for d in self.all_data if "ran" not in os.path.basename(d)]
    ...
    def __getitem__(self, idx):
        ...
        img_type = img_name.split(self.phase+"/")[1].split("/")[0]
...
image_datasets = {x: StatusDataset(data_dir, x,
                                   data_transforms[x])
                  for x in splits}
